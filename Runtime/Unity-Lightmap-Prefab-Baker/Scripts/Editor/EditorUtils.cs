...
namespace PrefabLightMapBaker
{
    public static class EditorUtils
    {
        ...
        public static Texture2D SaveLightmapAsset(string copyFrom, string saveTo)
        {
            ...
            int firstIndexOf = saveTo.IndexOf("Assets/");
            var relative = saveTo.Substring(firstIndexOf);
            try
            {
                Directory.CreateDirectory(Directory.GetParent(saveTo).FullName);
                AssetDatabase.CreateAsset(lightMapCopy, relative);
            }
            catch
            {
            ...
            // Refresh
            lightMapCopy = AssetDatabase.LoadAssetAtPath<Texture2D>(relative);
